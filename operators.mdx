...
8. The Operator returns the SelectionResult (or `no_match`) to the Platform.
...
Used by Brand Agents to submit responses.
...
- Match against platform `aip.json` editorial_domains
...
The mode is included in the SelectionResult.
...
- **Delegation started**: session handoff occurred (delegate mode)
...
| Platform SDK | Yes | Provide a frontend SDK for platforms |
...
* **Platform rendering support**  -  Provide a SDK for rendering and event tracking
