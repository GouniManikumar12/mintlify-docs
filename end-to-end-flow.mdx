...
sequenceDiagram
    ...
    Note over Platform,BrandAgent: After session start, live task turns may flow directly
    User->>Platform: Follow-up questions in delegated session
    Platform->>BrandAgent: Delegated task turns
    BrandAgent->>Platform: Delegated task responses
    Platform->>Operator: delegation_activity events
    BrandAgent->>Operator: delegation_activity events
    Note over Platform,BrandAgent: New task data is collected in-session and is session-bound by default
...
- `source_request_id`: Reference to the originating PlatformRequest `request_id`
- `timestamp`: ISO 8601 timestamp
...
This event proves session initiation and authorization. It does not imply that every subsequent delegated turn is transported through the Operator.
...
request_id (PlatformRequest)
    ↓
context_id (ContextRequest) ← references request_id via source_request_id
    ↓
