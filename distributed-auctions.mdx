...
3. **Window** – A 30–70 ms timer starts; bidders subscribe to their pools and push signed bids to `POST /aip/bid`.
4. **Collect** – The Operator drains the bid inbox the moment the window expires and runs operator-defined selection logic with deterministic tie-breakers.
5. **Respond** – Platforms receive either a fully populated `PlatformResponse` or a `no_match` JSON document that conforms to the schema.
...
- Record `no_match` outcomes so Platforms can differentiate “no eligible bidders” from transport failures.
