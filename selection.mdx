...
The Operator MUST return a single `SelectionResult` (or `no_match` when no responses arrive in time) and a `serve_token` back to the platform. The result includes the selected agent and the determined interaction mode (recommend or delegate).
...
6. Platform receives a single `SelectionResult` with `serve_token`.
