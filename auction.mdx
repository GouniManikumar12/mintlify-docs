...
The Operator MUST return a single `PlatformResponse` and a `serve_token` back to the Platform. When no responses arrive in time, the response uses `status: "no_fill"`. The protocol guarantees deterministic settlement through flow-specific billing paths, but those paths do not dictate auction ranking.
