...
The Operator MUST return a single `PlatformResponse` and a `serve_token` back to the Platform. When no responses arrive in time, the response uses `status: "no_fill"`. The protocol guarantees deterministic settlement based on the event ladder (CPA > CPC > CPX).
