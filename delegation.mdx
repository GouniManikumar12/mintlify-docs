...
When the Operator selects delegate mode, the selection result includes a delegation object:
...
sequenceDiagram
  ...
  Operator->>Platform: SelectionResult (mode: delegate, delegation object)
  ...
  Operator->>BrandAgent: Session initiated (scoped context)
  Note over Platform,BrandAgent: Operator connects Platform and BrandAgent for a multi-turn session
...
4. **Session tracking.** The `session_id` in the delegation object links all events back to the original selection and serve token.
5. **Multi-turn session.** Once the session is initiated, the user may ask multiple questions. The Brand Agent responds within the delegated session on behalf of the AI platform while representing its brand.
...
| `delegation_started` | User consents and session begins | Brand Agent (via Operator) |
...
6. User confirms
7. QuickBooks agent receives scoped context: `["intent", "constraints"]`
8. QuickBooks agent initiates signup flow
9. User completes signup
10. QuickBooks agent fires `task_completed` with conversion details
11. Operator verifies the outcome against the serve token
12. Settlement: CPA is charged and recorded
