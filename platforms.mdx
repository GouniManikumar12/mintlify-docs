...
4. **Run the delegated session (multi-turn)**  -  During the session, the user can ask multiple questions and the Brand Agent responds within that session while representing its brand, with the Operator mediating the handoff.
5. **Track outcomes and expiration**  -  The Platform fires `exposure_shown` and any applicable `interaction_started` events. The Operator records `delegation_started`, and the Brand Agent submits `task_completed` when the task completes. If the session expires without completion, the Operator handles expiration according to operator-defined rules and settlement policy.
