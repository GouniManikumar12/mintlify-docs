...
<AccordionGroup>
  ...
  <Accordion title="How does delegation consent work?">
    When the Operator returns a SelectionResult with delegate mode, the platform must present the offer to the user and collect explicit consent before any session transfer occurs. If the user declines, no session is created. The protocol does not prescribe the consent UI.
  </Accordion>
