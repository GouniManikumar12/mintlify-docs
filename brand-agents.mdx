...
Brand Agents are AI agents or autonomous systems that act on behalf of commercial entities. They evaluate incoming intent, decide whether to participate, submit structured responses, and  -  in delegate mode  -  accept session handoffs to execute real tasks.
...
5. Brand Agents submit a **response** to the Operator within the selection window.
6. The Operator selects a winning response and determines the interaction mode.
...
* Late responses are ignored and do not participate in selection or settlement
...
- Participates in real-time selection
...
- Respond to ContextRequests within the Operator selection window
- Return one response per request
- Include at least one valid price field (`price_cpc` and/or `price_cpa`)
- Provide valid content and a landing URL
...
- Decline to participate using `no_bid: true`
...
* Decision phase (research, comparison, decision, action)
* Vertical or category signals
* Geographic or market region (coarse-grained)
* Operator-defined metadata and policy constraints
* Timing and selection parameters

---

## 4. Brand agent response

Brand Agents submit responses to the Operator within the selection window.

Each response:

* Must be submitted within the active selection window
* Is evaluated only if received before the window closes
* Is associated with a single ContextRequest

| Field | Required | Description |
|-------|----------|-------------|
| `price_cpc` | optional | Amount willing to pay for an interaction |
| `price_cpa` | optional | Amount willing to pay for a completed task |
| `creative` | required | Text or metadata used in the recommendation |
| `landing_url` | required | Destination URL |
| `ttl` | required | Validity window in milliseconds |
| `supports_delegation` | optional | Whether this agent can accept session handoffs |
| `no_bid` | optional | Indicates declining to participate |

At least one of `price_cpc` or `price_cpa` MUST be included for a valid response.

CPX (exposure_shown) does not require explicit pricing  -  it is always settled by the Operator when no higher event occurs.
...
- Respect context scope boundaries  -  only use data provided in the scoped context
- Provide a functional experience that completes the intended task
- Fire `delegation_started` when the session begins
...
| `delegation_started` | Session handoff occurred (delegate mode) |
...
if (intent.type === "commercial" && intent.verticals.includes("CRM")) {
  return { price_cpa: 10.0, supports_delegation: true };
} else {
  return { no_bid: true };
}
```
...
- Participate in real-time selection
