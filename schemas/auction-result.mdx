...
If `status` is `"no_fill"`, `winner` and `render` MUST be omitted.
...
| `status` | string | Yes | Auction outcome: `filled`, `no_fill`, or `error` |
...
- If `status = "no_fill"`, `winner` and `render` MUST be omitted
