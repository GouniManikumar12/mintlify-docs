...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `allow_raw_query_downstream` | boolean | Yes | Whether the raw query text may be forwarded |
| `allow_message_history_downstream` | boolean | Yes | Whether message history may be forwarded |
...
```json
{
  ...
  "consent": {
    ...
    "constraints": {
      "allow_raw_query_downstream": false,
      "allow_message_history_downstream": false,
...
- `consent.constraints` flags must be respected when forwarding data to brand agents or downstream systems.
