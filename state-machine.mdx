...
| EXPOSURE_SHOWN | DELEGATION_STARTED | `delegation_started` event | Operator (may use platform and agent signals) |
...
1. Selection completes, `serve_token` generated → state = *PENDING*
2. Operator verifies a **exposure_shown** event → state = *EXPOSURE_SHOWN*
3. User consents, **delegation_started** event verified → state = *DELEGATION_STARTED*
